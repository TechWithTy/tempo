"""
Centralized exception handling for Tempo service.

Defines:
- Base API Exception
- Standard error responses
- Common HTTP exceptions
- Tempo-specific business logic exceptions
"""
...
class TempoStorageError(APIError):
    """500 - Error storing or retrieving telemetry data"""

    def __init__(self, operation: str, reason: str = None, details: dict[str, Any] = None):
        super().__init__(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            error_code="tempo_storage_error",
            message=f"Failed to {operation} telemetry data"
                    + (f": {reason}" if reason else ""),
            details=details,
        )


class TempoIngestionError(APIError):
    """422 - Error ingesting telemetry data"""

    def __init__(self, reason: str = None, details: dict[str, Any] = None):
        super().__init__(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            error_code="tempo_ingestion_error",
